...
counter = Counter(lst)
max_len_key = max(map(lambda x: len(x), lst))
for key,value in sorted(counter.items()):
    key_ord = sum(map(lambda x:ord(x),list(key.replace(' ',''))))
    print(f'{key.ljust(max_len_key)}: {key_ord} UC x {value} = {key_ord * value} UC')
