...
for key,value in data.items():
    if datetime.strptime(value[1],'%d.%m.%Y %H:%M:%S').timestamp() - datetime.strptime(value[0],'%d.%m.%Y %H:%M:%S').timestamp() <= min:
        min = datetime.strptime(value[1],'%d.%m.%Y %H:%M:%S').timestamp() - datetime.strptime(value[0],'%d.%m.%Y %H:%M:%S').timestamp()
        min_key[0]= key
print(*min_key)
