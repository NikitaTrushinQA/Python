...
#Дополните приведенный ниже код, чтобы он сгруппировал сотрудников по соответствующим отделам и вывел названия всех отделов,
# указав для каждого имена и фамилии его сотрудников. Отделы, а также имена и фамилии сотрудников в этих отделах,
...
for branch,name in staff_broken:
    company_info[branch].append(name)
for key in sorted(company_info,key=lambda x:(x[0])):
    print(f"{key}: {', '.join(sorted(list(set(company_info[key])), key=lambda x: x.split(' ')[0]))}")
