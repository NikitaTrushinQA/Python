...
#
# Итератор класса CardDeck должен генерировать последовательность из 52 игральных карт, а после возбуждать исключение StopIteration.
...
class CardDeck:
    ...
    def __init__(self):
        self.iterator = iter([(self.cards[j],self.suits[i]) for i in range(len(self.suits)) for j in range(len(self.cards))])
        self.index = -1
    def __iter__(self):
        return self
